...
const CampaignCard: React.FC<{
  ...
  const handleDelete = async () => {
    startTransition(async () => {
      try {
        await axios.delete(`${BASE_URL}/campaigns/delete/${campaign._id}`, {
          headers: {
  ...
  // Decide link target based on role
  let linkTo = `/home/campaigns/${campaign._id}`;
  if (campaigner) {
    linkTo = `/user/dashboard/campaigns/${campaign._id}`;
  } else if (admin) {
    linkTo = `/admin/campaigns/${campaign._id}`;
  }
  ...
  return (
    ...
    >
      <div className="font-sans flex flex-col border border-primarylight bg-white p-4 gap-4 rounded-lg overflow-hidden w-full max-w-[370px] hover:shadow-lg hover:border-primarylight transition-all duration-300">
        ...
        {campaign?.verified && (
          <p className="text-sm font-bold bg-primarylight w-[70px] text-center text-white p-1 rounded-full">
            {campaign?.verified ? "S18A" : ""}
          </p>
        )}
